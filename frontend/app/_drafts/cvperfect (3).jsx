...
// ─── CV DOCUMENT ──────────────────────────────────────────────────────────────
function CVDocument({ cvData, setCvData, color, photoUrl, onPhotoClick, editMode }) {
  ...
  const set = (key, val) => setCvData(p => ({ ...p, [key]: val }));
  const setNested = (arr, idx, field, val) => setCvData(p => { const a = JSON.parse(JSON.stringify(p[arr])); a[idx][field] = val; return { ...p, [arr]: a }; });
  const setList = (arr, idx, val) => setCvData(p => { const a = [...p[arr]]; a[idx] = val; return { ...p, [arr]: a }; });
    ...
    <div id="cv-document" style={{ fontFamily: "'Segoe UI', Arial, sans-serif", width: 794, background: "#fff", color: "#1a1a1a" }}>
        ...
        <div onClick={onPhotoClick} style={{ flexShrink: 0, cursor: "pointer" }}>
          {photoUrl
            ...
            : <div style={{ width: 96, height: 96, borderRadius: "50%", background: "rgba(255,255,255,0.15)", border: "3px dashed rgba(255,255,255,0.5)", display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center" }}>
                ...
                <span style={{ fontSize: 9, color: "rgba(255,255,255,0.8)", marginTop: 3 }}>Foto</span>
              </div>}
        </div>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 258px" }}>
        ...
        <div style={{ padding: "22px 26px 22px 42px" }}>
          <Sec title="Profil Profesional" color={color}>
            <F v={cvData.despre} onCh={v => set("despre", v)} multi style={{ fontSize: 13, lineHeight: 1.7, color: "#444", display: "block", width: "100%" }} ph="Descrie-te profesional..." />
          </Sec>
          <Sec title="Experiență Profesională" color={color}>
            {cvData.experienta.map((exp, i) => (
              <div key={i} style={{ marginBottom: 16 }}>
                <div style={{ display: "flex", justifyContent: "space-between", gap: 8 }}>
                  <div style={{ flex: 1, minWidth: 0 }}>
                    <F v={exp.rol} onCh={v => setNested("experienta", i, "rol", v)} style={{ display: "block", fontWeight: 700, fontSize: 13.5, color: "#111" }} />
                    <F v={exp.firma} onCh={v => setNested("experienta", i, "firma", v)} style={{ display: "block", color, fontWeight: 600, fontSize: 12.5 }} />
                  </div>
                  <div style={{ background: color, color: "#fff", padding: "2px 9px", borderRadius: 20, fontSize: 11, whiteSpace: "nowrap", flexShrink: 0, alignSelf: "flex-start" }}>
                    <F v={exp.perioada} onCh={v => setNested("experienta", i, "perioada", v)} style={{ color: "#fff" }} />
                  </div>
                </div>
                {editMode
                  ? <EF value={exp.desc} onChange={v => setNested("experienta", i, "desc", v)} multiline style={{ fontSize: 12.5, color: "#555", marginTop: 6 }} placeholder="Realizări separate cu •" />
          ...
          <Sec title="Educație" color={color}>
            {cvData.educatie.map((edu, i) => (
              <div key={i} style={{ marginBottom: 10, display: "flex", justifyContent: "space-between", gap: 8 }}>
                <div style={{ flex: 1 }}>
                  <F v={edu.diploma} onCh={v => setNested("educatie", i, "diploma", v)} style={{ display: "block", fontWeight: 700, fontSize: 13 }} />
                  <F v={edu.institutie} onCh={v => setNested("educatie", i, "institutie", v)} style={{ display: "block", color, fontSize: 12.5 }} />
                </div>
                <F v={edu.perioada} onCh={v => setNested("educatie", i, "perioada", v)} style={{ fontSize: 11.5, color: "#888", whiteSpace: "nowrap", flexShrink: 0 }} />
              </div>
            ))}
          </Sec>
        </div>
        ...
        <div style={{ background: "#f7f8fa", padding: "22px 18px", borderLeft: "1px solid #eee" }}>
          <SideSec title="Competențe" color={color}>
            {cvData.competente.map((c, i) => {
              if (!skillWidths.current[c]) skillWidths.current[c] = 80;
              return (
                <div key={i} style={{ marginBottom: 9 }}>
                  {editMode
                    ? <EF value={c} onChange={v => setList("competente", i, v)} style={{ fontSize: 12, fontWeight: 500, color: "#444" }} />
                    : <span style={{ fontSize: 12, fontWeight: 500, color: "#444" }}>{c}</span>}
          ...
          <SideSec title="Limbi Străine" color={color}>
            {cvData.limbi.map((l, i) => (
              <div key={i} style={{ display: "flex", alignItems: "center", gap: 7, marginBottom: 7 }}>
                <span style={{ width: 7, height: 7, borderRadius: "50%", background: color, flexShrink: 0, display: "inline-block" }} />
                {editMode
                  ? <EF value={l} onChange={v => setList("limbi", i, v)} style={{ fontSize: 12.5, color: "#555" }} />
                  : <span style={{ fontSize: 12.5, color: "#555" }}>{l}</span>}
              </div>
            ))}
          </SideSec>
          <SideSec title="Certificări" color={color}>
            {cvData.certificari.map((c, i) => (
              <div key={i} style={{ marginBottom: 7, padding: "5px 9px", background: "#fff", borderRadius: 5, borderLeft: `3px solid ${color}` }}>
                {editMode
                  ? <EF value={c} onChange={v => setList("certificari", i, v)} style={{ fontSize: 11.5, color: "#555" }} />
                  : <span style={{ fontSize: 11.5, color: "#555" }}>{c}</span>}
              </div>
            ))}
          </SideSec>
        </div>
...
function TemplateCard({ template, onSelect }) {
  ...
  return (
    <div onMouseEnter={() => setHov(true)} onMouseLeave={() => setHov(false)}
      ...
      <div style={{ background: `linear-gradient(135deg, ${color} 0%, ${color}cc 100%)`, padding: "18px 18px 14px", position: "relative" }}>
...
export default function App() {
  ...
  const filtered = cvTemplates.filter(t => t.job.toLowerCase().includes(search.toLowerCase()) || t.data.titlu.toLowerCase().includes(search.toLowerCase()));

  const select = (t) => { setTmpl(t); setCvData(JSON.parse(JSON.stringify(t.data))); setPhoto(null); setEditMode(false); setPage("editor"); window.scrollTo({ top: 0 }); };
  ...
  const exportPDF = async () => {
    ...
    try {
      await window.html2pdf().set({
        margin: 0,
        filename: `CV_${(cvData.nume || "CV").replace(/ /g, "_")}.pdf`,
  ...
  const saveData = () => { setSaved(true); setTimeout(() => setSaved(false), 2200); };
  ...
  return (
    <div style={{ minHeight: "100vh", background: "linear-gradient(135deg, #f0f4ff 0%, #faf8ff 100%)", fontFamily: "'Segoe UI', system-ui, sans-serif" }}>
      ...
      <header style={{ background: "#fff", borderBottom: "1px solid #e8ecf4", position: "sticky", top: 0, zIndex: 999, boxShadow: "0 2px 14px rgba(0,0,0,0.07)" }}>
        <div style={{ maxWidth: 1240, margin: "0 auto", padding: "0 20px", height: 58, display: "flex", alignItems: "center", justifyContent: "space-between" }}>
          <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
          ...
          {page === "editor" && tmpl && cvData && (
            <div style={{ display: "flex", gap: 7, alignItems: "center" }}>
              <button onClick={() => setPage("grid")} style={btnStyle("#e2e8f0", "#fff", "#374151")}>← Template-uri</button>
              <button onClick={() => fileRef.current.click()} style={btnStyle("#bae6fd", "#f0f9ff", "#0369a1")}>{photo ? "🔄 Schimbă foto" : "📷 Adaugă foto"}</button>
              <button onClick={() => { setEditMode(e => !e); }} style={btnStyle(editMode ? "#fcd34d" : "#e2e8f0", editMode ? "#fffbeb" : "#fff", editMode ? "#92400e" : "#374151", editMode ? 700 : 600)}>
                {editMode ? "👁 Previzualizare" : "✏️ Editează CV"}
              </button>
              {editMode && <button onClick={saveData} style={btnStyle(saved ? "#059669" : "#bbf7d0", saved ? "#059669" : "#f0fdf4", saved ? "#fff" : "#059669", 700)}>{saved ? "✓ Salvat!" : "💾 Salvează"}</button>}
              <button onClick={exportPDF} disabled={exporting}
                style={{ padding: "7px 18px", borderRadius: 8, background: exporting ? "#94a3b8" : `linear-gradient(135deg,${tmpl.color},#7c3aed)`, color: "#fff", border: "none", cursor: exporting ? "not-allowed" : "pointer", fontWeight: 800, fontSize: 12.5, boxShadow: exporting ? "none" : `0 3px 10px ${tmpl.color}44` }}>
                {exporting ? "⏳ Generare..." : "⬇️ Descarcă PDF"}
              </button>
            </div>
          )}
      ...
      {page === "editor" && editMode && (
        <div style={{ background: "#fef9c3", borderBottom: "1.5px solid #fde047", padding: "7px 20px", textAlign: "center" }}>
          <span style={{ fontSize: 13, color: "#854d0e", fontWeight: 600 }}>✏️ Mod editare — Click pe orice câmp din CV pentru a-l modifica live</span>
        </div>
      )}

      {/* ── GRID ── */}
      {page === "grid" && (
        <div style={{ maxWidth: 1240, margin: "0 auto", padding: "34px 20px" }}>
          <div style={{ textAlign: "center", marginBottom: 40 }}>
            <div style={{ display: "inline-block", background: "linear-gradient(135deg,#eff6ff,#faf5ff)", border: "1px solid #c7d2fe", borderRadius: 20, padding: "4px 14px", fontSize: 12, color: "#4338ca", fontWeight: 600, marginBottom: 12 }}>✦ 18 Template-uri Profesionale</div>
            <h1 style={{ fontSize: 38, fontWeight: 900, color: "#0f172a", margin: "0 0 10px", letterSpacing: "-1px", lineHeight: 1.2 }}>
              Crează CV-ul Perfect<br /><span style={{ background: "linear-gradient(135deg,#1a56db,#7c3aed)", WebkitBackgroundClip: "text", WebkitTextFillColor: "transparent" }}>în 5 Minute</span>
            </h1>
            <p style={{ fontSize: 15.5, color: "#64748b", margin: "0 auto 24px", maxWidth: 480, lineHeight: 1.65 }}>Editare live a tuturor câmpurilor + export PDF real. Standard european, ATS optimizat.</p>
            <div style={{ display: "flex", justifyContent: "center", gap: 40, marginBottom: 32 }}>
              {[["18","Template-uri"],["✏️","Editare Live"],["PDF","Export Real"],["ATS","Optimizat"]].map(([v,l]) => (
      ...
      {/* ── EDITOR ── */}
      {page === "editor" && tmpl && cvData && (
        <div style={{ maxWidth: 1240, margin: "0 auto", padding: "22px 20px", display: "grid", gridTemplateColumns: "1fr 295px", gap: 18, alignItems: "start" }}>
          {/* CV */}
          <div style={{ background: "#fff", borderRadius: 14, overflow: "hidden", boxShadow: "0 6px 28px rgba(0,0,0,0.09)", border: `2px solid ${editMode ? "#fcd34d" : "#e8ecf4"}`, transition: "border 0.2s" }}>
            <CVDocument cvData={cvData} setCvData={setCvData} color={tmpl.color} photoUrl={photo} onPhotoClick={() => fileRef.current.click()} editMode={editMode} />
          </div>

          {/* PANEL */}
          <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
            {/* Main actions */}
            <div style={{ background: "#fff", border: `1.5px solid ${editMode ? "#fcd34d" : "#e8ecf4"}`, borderRadius: 14, padding: 18, transition: "all 0.2s" }}>
              <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 14 }}>
                <span style={{ fontSize: 22 }}>{tmpl.icon}</span>
                <div>
                  <div style={{ fontWeight: 700, fontSize: 14, color: "#0f172a" }}>{tmpl.job}</div>
                  <div style={{ fontSize: 11, color: "#94a3b8" }}>Template #{tmpl.id}</div>
                </div>
              </div>

              <div style={{ background: editMode ? "#fef9c3" : "#f8fafc", borderRadius: 9, padding: "10px 12px", fontSize: 12.5, color: editMode ? "#854d0e" : "#475569", lineHeight: 1.65, marginBottom: 14 }}>
                {editMode
                  ? <><strong>Mod editare activ:</strong><br />• Click pe orice câmp din CV<br />• Modifică direct în document<br />• Experiența: separă cu " • "<br />• Salvează când ești gata ✓</>
                  : <><strong>CV-ul tău este gata!</strong><br />Apasă <strong>Editează</strong> pentru a personaliza datele, sau <strong>Descarcă PDF</strong> pentru a-l salva.</>}
              </div>

              <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
                <button onClick={() => fileRef.current.click()} style={{ ...fullBtn, background: photo ? "#f0fdf4" : "#f0f9ff", color: photo ? "#059669" : "#0369a1", border: `1.5px solid ${photo ? "#bbf7d0" : "#bae6fd"}` }}>
                ...
                <button onClick={() => setEditMode(e => !e)} style={{ ...fullBtn, background: editMode ? "#fffbeb" : "#f8fafc", color: editMode ? "#b45309" : "#374151", border: `1.5px solid ${editMode ? "#fcd34d" : "#e2e8f0"}`, fontWeight: 700 }}>
                  {editMode ? "👁 Ieși din editare" : "✏️ Editează CV-ul"}
                </button>
                {editMode && <button onClick={saveData} style={{ ...fullBtn, background: saved ? "#059669" : "#f0fdf4", color: saved ? "#fff" : "#059669", border: `1.5px solid ${saved ? "#059669" : "#bbf7d0"}`, fontWeight: 700, transition: "all 0.2s" }}>{saved ? "✓ Date salvate!" : "💾 Salvează modificările"}</button>}
                <button onClick={exportPDF} disabled={exporting}
                  style={{ ...fullBtn, background: exporting ? "#94a3b8" : `linear-gradient(135deg,${tmpl.color},#7c3aed)`, color: "#fff", border: "none", fontWeight: 800, fontSize: 14, cursor: exporting ? "not-allowed" : "pointer", boxShadow: exporting ? "none" : `0 4px 14px ${tmpl.color}44`, padding: "12px" }}>
                  {exporting ? "⏳ Se generează PDF-ul..." : "⬇️ Descarcă CV (PDF)"}
                </button>
              </div>
            </div>
            ...
            {editMode && (
              <div style={{ background: "#fff", border: "1.5px solid #e8ecf4", borderRadius: 14, padding: 16 }}>
                <div style={{ fontSize: 12, fontWeight: 700, color: "#64748b", textTransform: "uppercase", letterSpacing: 0.6, marginBottom: 12 }}>Date de Contact</div>
                {[["Nume complet","nume"],["Titlu profesional","titlu"],["Email","email"],["Telefon","telefon"],["Oraș","oras"],["LinkedIn URL","linkedin"]].map(([label, key]) => (
            ...
            <div style={{ background: "#fff", border: "1.5px solid #e8ecf4", borderRadius: 14, padding: 15 }}>
              ...
              <div style={{ display: "flex", flexDirection: "column", gap: 7 }}>
                {cvTemplates.filter(t => t.id !== tmpl.id).slice(0, 5).map(t => (
      ...
      <footer style={{ borderTop: "1px solid #e8ecf4", background: "#fff", padding: "18px 20px", textAlign: "center", marginTop: 36 }}>
        <p style={{ margin: 0, fontSize: 12, color: "#94a3b8" }}>© 2025 CVPerfect.online — 18 Template-uri Standard European | Editare Live | Export PDF Real | ATS Optimizat</p>
      </footer>
    </div>
  );
}

const fullBtn = { width: "100%", padding: "9px 14px", borderRadius: 9, cursor: "pointer", fontWeight: 600, fontSize: 12.5, textAlign: "center", boxSizing: "border-box" };
const btnStyle = (borderColor, bg, color, fw = 600) => ({ padding: "7px 13px", borderRadius: 8, border: `1.5px solid ${borderColor}`, background: bg, color, cursor: "pointer", fontWeight: fw, fontSize: 12.5 });
