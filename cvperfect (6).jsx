...
const API_URL = "https://cvperfect-14.onrender.com";
...
function useStripePayment() {
  ...
  useEffect(() => {
    ...
    if (params.get("payment") === "success" && params.get("session_id")) {
      setChecking(true);
      fetch(`${API_URL}/verify-payment?session_id=${params.get("session_id")}`)
        .then(r => r.json()).then(d => { if (d.paid) { setPaid(true); window.history.replaceState({}, "", window.location.pathname); } })
        .catch(console.error).finally(() => setChecking(false));
    }
  }, []);
  const startPayment = async (templateName, lang) => {
    try {
      const res = await fetch(`${API_URL}/create-checkout`, { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ templateName, lang }) });
      const data = await res.json();
      if (data.url) window.location.href = data.url;
      else alert("Eroare la inițierea plății. Încearcă din nou.");
    } catch (e) { alert("Nu s-a putut conecta la server."); }
  };
  return { paid, checking, startPayment };
}

// ─── PAYWALL MODAL ───────────────────────────────────────────────────────────
function PaywallModal({ onClose, onPay, templateName, color }) {
  return (
    <div style={{ position: "fixed", inset: 0, background: "rgba(15,23,42,0.7)", zIndex: 9999, display: "flex", alignItems: "center", justifyContent: "center", padding: 20 }}>
      <div style={{ background: "#fff", borderRadius: 20, padding: 32, maxWidth: 420, width: "100%", boxShadow: "0 24px 64px rgba(0,0,0,0.25)", position: "relative" }}>
        <button onClick={onClose} style={{ position: "absolute", top: 16, right: 16, background: "#f1f5f9", border: "none", width: 32, height: 32, borderRadius: "50%", cursor: "pointer", fontSize: 16 }}>✕</button>
        ...
        <p style={{ margin: "0 0 24px", fontSize: 14, color: "#64748b", textAlign: "center", lineHeight: 1.6 }}>CV-ul tău profesional <strong>{templateName}</strong> este gata de descărcat.</p>
        <div style={{ background: "linear-gradient(135deg, #f0f9ff, #eff6ff)", border: "1.5px solid #bfdbfe", borderRadius: 14, padding: "20px 24px", marginBottom: 20, textAlign: "center" }}>
          ...
          <div style={{ fontSize: 42, fontWeight: 900, color: "#0f172a" }}>19 <span style={{ fontSize: 22 }}>RON</span></div>
          <div style={{ fontSize: 12, color: "#94a3b8", marginTop: 4 }}>plată unică · fără abonament</div>
        </div>
        {["✅ PDF profesional format A4 european","✅ Optimizat ATS — trecut prin filtre HR","✅ Descărcare instant după plată","✅ Plată securizată prin Stripe 🔒"].map((b,i) => <div key={i} style={{ fontSize: 13, color: "#374151", marginBottom: 8 }}>{b}</div>)}
        <button onClick={onPay} style={{ width: "100%", padding: "14px", borderRadius: 12, background: `linear-gradient(135deg, ${color}, #7c3aed)`, color: "#fff", border: "none", cursor: "pointer", fontWeight: 800, fontSize: 16, marginTop: 16, marginBottom: 12 }}>
          💳 Plătește 19 RON & Descarcă
        </button>
        <p style={{ margin: 0, fontSize: 11, color: "#94a3b8", textAlign: "center" }}>🔒 Plată securizată prin Stripe · Visa, Mastercard, Google Pay</p>
      </div>
    </div>
  );
}
...
function EF({ value, onChange, multiline, style, placeholder }) {
  ...
  const ref = useRef();
  useEffect(() => { if (active && ref.current) ref.current.focus(); }, [active]);
  const base = { ...style, outline: "none", background: active ? "rgba(253,224,71,0.28)" : "transparent", border: active ? "1.5px dashed #f59e0b" : "1.5px dashed transparent", borderRadius: 3, padding: "1px 4px", transition: "all 0.15s", cursor: "text", fontFamily: "inherit", fontSize: "inherit", fontWeight: "inherit", color: "inherit", lineHeight: "inherit" };
  if (multiline) return <textarea ref={ref} value={value} placeholder={placeholder} onChange={e => onChange(e.target.value)} onFocus={() => setActive(true)} onBlur={() => setActive(false)} rows={4} style={{ ...base, width: "100%", resize: "vertical", display: "block", boxSizing: "border-box" }} />;
  return <input ref={ref} type="text" value={value} placeholder={placeholder} onChange={e => onChange(e.target.value)} onFocus={() => setActive(true)} onBlur={() => setActive(false)} style={{ ...base, width: "100%", display: "inline-block", boxSizing: "border-box" }} />;
}
...
function CVDocument({ cvData, setCvData, color, photoUrl, onPhotoClick, editMode }) {
  ...
  const F = editMode
    ? ({ v, onCh, multiline, style, ph }) => <EF value={v} onChange={onCh} multiline={multiline} style={style} placeholder={ph} />
    ...
    <div id="cv-document" style={{ fontFamily: "'Segoe UI', Arial, sans-serif", width: 794, background: "#fff", color: "#1a1a1a" }}>
      <div style={{ background: color, padding: "30px 42px 22px", display: "flex", alignItems: "center", gap: 24 }}>
        <div onClick={onPhotoClick} style={{ flexShrink: 0, cursor: "pointer" }}>
          {photoUrl ? <img src={photoUrl} alt="foto" style={{ width: 96, height: 96, borderRadius: "50%", objectFit: "cover", border: "3px solid rgba(255,255,255,0.4)" }} />
            : <div style={{ width: 96, height: 96, borderRadius: "50%", background: "rgba(255,255,255,0.15)", border: "3px dashed rgba(255,255,255,0.5)", display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center" }}>
                <span style={{ fontSize: 24 }}>📷</span><span style={{ fontSize: 9, color: "rgba(255,255,255,0.8)", marginTop: 3 }}>Foto</span>
              </div>}
        </div>
        <div style={{ flex: 1, minWidth: 0 }}>
          <F v={cvData.nume} onCh={v => set("nume", v)} style={{ display: "block", color: "#fff", fontSize: 25, fontWeight: 700 }} />
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 258px" }}>
        <div style={{ padding: "22px 26px 22px 42px" }}>
          <Sec title="Profil Profesional" color={color}>
            <F v={cvData.despre} onCh={v => set("despre", v)} multiline style={{ fontSize: 13, lineHeight: 1.7, color: "#444", display: "block", width: "100%" }} ph="Profil profesional..." />
          </Sec>
...
export default function App() {
  ...
  const { paid, checking, startPayment } = useStripePayment();
  ...
  const select = (t) => {
    setTmpl(t); setCvData(JSON.parse(JSON.stringify(t.data)));
    setPhoto(null); setEditMode(false); setPage("editor");
  ...
  const exportPDF = async () => {
    ...
    const filename = `CV_${(cvData?.nume || "CV").replace(/ /g, "_")}.pdf`;
    try {
    ...
    } catch(e) { alert("Eroare la export. Încearcă din nou."); }
    setExporting(false);
  };

  const handleDownloadClick = () => { if (paid) exportPDF(); else setShowPaywall(true); };
  const handlePayNow = () => { setShowPaywall(false); startPayment(tmpl?.job || "CV", "ro"); };

  return (
    <div style={{ minHeight: "100vh", background: "linear-gradient(135deg, #f0f4ff 0%, #faf8ff 100%)", fontFamily: "'Segoe UI', system-ui, sans-serif" }}>

      {showPaywall && tmpl && <PaywallModal onClose={() => setShowPaywall(false)} onPay={handlePayNow} templateName={tmpl.job} color={tmpl.color} />}

      {paid && (
        <div style={{ background: "linear-gradient(90deg, #059669, #0d9488)", padding: "10px 20px", textAlign: "center" }}>
          <span style={{ fontSize: 14, color: "#fff", fontWeight: 700 }}>🎉 Plată confirmată! Apasă butonul PDF pentru a descărca.</span>
        </div>
      )}
      {checking && (
        <div style={{ background: "#fef9c3", padding: "8px 20px", textAlign: "center" }}>
          ...
          {page === "editor" && tmpl && cvData && (
            <div style={{ display: "flex", gap: 6, alignItems: "center", flexWrap: "wrap", justifyContent: "flex-end" }}>
              ...
              <button onClick={handleDownloadClick} disabled={exporting}
                style={{ padding: "7px 16px", borderRadius: 8, background: paid ? "linear-gradient(135deg,#059669,#0d9488)" : exporting ? "#94a3b8" : `linear-gradient(135deg,${tmpl.color},#7c3aed)`, color: "#fff", border: "none", cursor: exporting ? "not-allowed" : "pointer", fontWeight: 800, fontSize: 12.5 }}>
                {exporting ? "⏳..." : paid ? "⬇️ Descarcă PDF ✓" : "🔒 PDF — 19 RON"}
              </button>
            </div>
          )}
      ...
      {page === "grid" && (
        <div style={{ maxWidth: 1260, margin: "0 auto", padding: "34px 20px" }}>
          <div style={{ textAlign: "center", marginBottom: 40 }}>
            <div style={{ display: "inline-block", background: "linear-gradient(135deg,#eff6ff,#faf5ff)", border: "1px solid #c7d2fe", borderRadius: 20, padding: "4px 14px", fontSize: 12, color: "#4338ca", fontWeight: 600, marginBottom: 12 }}>✦ 18 Template-uri · Standard European · ATS</div>
            ...
            <p style={{ fontSize: 15.5, color: "#64748b", margin: "0 auto 24px", maxWidth: 500, lineHeight: 1.65 }}>
              Editare live · Export PDF real · Plată securizată 19 RON
            </p>
            <div style={{ display: "flex", justifyContent: "center", gap: 36, marginBottom: 32 }}>
              {[["18","Template-uri"],["✏️","Editare Live"],["🔒","Plată Sigură"],["PDF","Export Real"]].map(([v,l]) => (
      ...
      {/* EDITOR */}
      {page === "editor" && tmpl && cvData && (
        <div style={{ maxWidth: 1260, margin: "0 auto", padding: "22px 20px", display: "grid", gridTemplateColumns: "1fr 295px", gap: 18, alignItems: "start" }}>
            ...
            <div style={{ background: "#fff", border: `1.5px solid ${editMode ? "#fcd34d" : "#e8ecf4"}`, borderRadius: 14, padding: 18 }}>
              ...
              <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
                ...
                <button onClick={handleDownloadClick} disabled={exporting}
                  ...
                  {exporting ? "⏳ Generare PDF..." : paid ? "⬇️ Descarcă PDF (plătit ✓)" : "🔒 Descarcă PDF — 19 RON"}
                </button>
              </div>
            </div>
      ...
      <footer style={{ borderTop: "1px solid #e8ecf4", background: "#fff", padding: "18px 20px", textAlign: "center", marginTop: 36 }}>
        <p style={{ margin: 0, fontSize: 12, color: "#94a3b8" }}>© 2025 CVPerfect.online · 18 Template-uri · Standard European · Export PDF · ATS Optimizat</p>
      </footer>
    </div>
  );
}
